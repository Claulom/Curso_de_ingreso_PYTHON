...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        #numeros
        precio_num = float(importe_txt)
        descuento_num = float(descuento_txt)

        descuento = descuento_num * precio_num / 100
        precio_actualizado = "El precio ${0} con un descuento del {1}% quedaria en ${2}".format(precio_num,descuento_num, precio_num-descuento)

        alert(title="Resultado", message= precio_actualizado)
